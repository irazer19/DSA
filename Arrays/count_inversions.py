...
def brute_foce(arr):
    """
    The brute force solution would involve checking every possible pair of indices (i,j) where i < j and counting when array[i] > array[j]
...
def merge(array, start, mid, end):
    ...
    while i <= mid and j <= end:
        ...
        if aux[i] <= aux[j]:
            ...
            # If array[i] > array[j], then all the elements which are after array[i] are also greater.
            # So array[j] will have to skip (inversions) all those elements to reach its correct spot.
            inversions += mid - i + 1
